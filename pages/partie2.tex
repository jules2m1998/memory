\part{ÉTUDE DÉTAILLÉE ET RÉALISATION}
...
\textit{Comme tout problème qui a été clairement défini, il faut le réaliser, mais nous ne pouvons
	...
	nous allons cadrer le projet en utilisant la méthode  du \ac{cps}, suivi d’une analyse
...
\begin{enumerate}
	\item \textbf{Les différents modèles d'estimation des coûts et ressources d’un projet}
	      ...
	      \begin{enumerate}
		      ...
		      \item \textbf{La méthode \ac{cocomo}} : est un modèle permettant de définir une estimation de l'effort à fournir dans un développement logiciel et la durée que ce dernier aura en fonction des ressources allouées.
	      \end{enumerate}
...
Pour la gestion du temps, il existe des méthodes de planification prévisionnelle de projet tel que le diagramme de \ac{gantt}. Ce dit diagramme regroupe toutes les tâches, les durées et les ressources ordonnées de manière graduelle permettant à toute l’équipe de suivre son évolution. Il peut être modifié au fur et à mesure en fonction des délais respectés ou pas ainsi que des imprévus. Dans le cadre de notre projet, la planification de nos tâches nous a permis de ressortir le diagramme de Gantt suivant :
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1.3\textwidth, height=0.7\textheight, angle=-90]{img/gantt}
...
\begin{table}[H]
	...
	\begin{tabular}{|l|p{7cm}|}
		\hline
		\multicolumn{1}{|l|}{Ordinateurs desktop} & 32 Go de RAM, Une carte graphique Nvidia GeForce GTX 1070, 250 Go de disque dur SSD, 1 To de disque dur HDD, Processeur Intel core i7 3.60GHz, Boitier et carte graphique predator, Ecran 32 pouces, Clavier HP \\ \hline
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1.3\textwidth, height=0.7\textheight, angle=-90]{img/ucd}
...
Chaque acteur de ce diagramme a certaines interactions bien spécifiques avec chaque module de l'application, interactions mieux illustrées dans les différents diagrammes des cas d'utilisations séparés par utilisateur suivants :

\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{img/aucd}
	\caption{Diagramme des cas d’utilisation d'un administrateur}
	\label{fig:mesh1}
\end{figure}

La figure ci-dessus présente l'administrateur et tous ses cas d'utilisation de la plateforme regroupé par module. Nous constatons que le champ d'action de l'administrateur s'étend aux modules de gestion des utilisateurs, des éléments chimiques, du matériel, mais aussi aux différents cas d'utilisation liés à un enseignant.

\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{img/tucd}
	\caption{Diagramme des cas d’utilisation d'un enseignant}
	\label{fig:mesh1}
\end{figure}

La figure ci-dessus présente l'enseignant et tous ses cas d'utilisation de la plateforme regroupé par module. Nous constatons que le champ d'action d'un enseignant s'étend aux modules de gestion des utilisateurs, des éléments chimiques et des réactions chimiques.


\begin{figure}[H]
	\centering
	\includegraphics{img/sucd}
	\caption{Diagramme des cas d’utilisation d'un apprenant}
	\label{fig:mesh1}
\end{figure}

La figure ci-dessus présente l'apprenant et tous ses cas d'utilisation de la plateforme regroupé par module. Nous constatons que le champ d'action d'un apprenant ne se limite qu'au module des réactions chimiques et uniquement au cas d'utilisation "effectuer une réaction chimique".
...
\begin{itemize}
	...
	\item \textbf{Changement d’état d'authentification d’un utilisateur }
	      ...
	      Ce diagramme présente de façon détaillée le changement d’état d’authentification d’un utilisateur en fonction d’évènements déclenchés sur la plateforme.
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Les \ac{cms} pour le web} :
		  ...
		  Il faudra donc mettre en place un serveur \ac{http} pour héberger son application. 
...
\textbf{ASP.net} pour le \textbf{backend}, \textbf{React} pour le \textbf{frontend} et \textbf{unreal engine} pour la \textbf{3D}. Comme \textbf{\ac{sgbd}}, nous allons utiliser \textbf{PostgreSQL} (le SGBD utilisé par l’entreprise).
...
\begin{figure}[H]
	...
	\includegraphics[trim={5cm 0 0 0}, width=1.2\textwidth, height=0.7\textheight]{img/dc}
...
\subsection{Quelques maquettes d'\ac{ihm}}
...
\subsubsection{Page de connexion des enseignants et administrateurs}
...
Cette interface représente une vue de dessus du laboratoire virtuelle ou les apprenants effectueront leurs réactions.
...
Il y apparait un local vu du dessus et présente l'environnement extérieur du laboratoire où auront lieu les réactions.

\subsubsection{Interface du laboratoire en vue de face}

Cette interface représente une vue de face du laboratoire virtuelle où les apprenants effectueront leurs réactions.
...
Il y apparait un local vu de face et présente l'environnement extérieur du laboratoire où auront lieu les réactions.

\subsubsection{Interface du laboratoire intérieur vue du fond}

Cette interface représente une vue intérieure du laboratoire vu du fond de la salle.
...
Il y apparait la table de l'apprenant avec la verrerie des réactions disposée dessus.
...
Le code backend d'un contrôleur permet de définir des réponses en fonction de requêtes effectuées par un client sur un serveur.

Le code ci-dessous présente le backend du contrôleur utilisateur, plus précisément la fonctionnalité de la modification des informations d'un utilisateur.
...
Le code ci-dessous présente le test unitaire de la fonctionnalité d'édition du profil utilisateur.

\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{img/utupdate}
	\caption{Code backend du test unitaire du cas d'utilisation de l'édition du profil utilisateur}
	\label{fig:mesh1}
\end{figure}

La figure ci-dessous présente le résultat du test unitaire du cas d'utilisation de l'édition du profil utilisateur.

\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{img/uutr}
	\caption{Résultat du test unitaire du cas d'utilisation de l'édition du profil utilisateur}
	\label{fig:mesh1}
\end{figure}
...
Le formulaire sur la figure ci-dessous permet la création d'un élément chimique (Atome et molécule) et cela en fonction de notre rôle, seul l'administrateur a le droit de créer des atomes tandis que la création des molécules est autorisée à tous les utilisateurs.
...
La figure ci-dessous présente le code du test unitaire de la fonctionnalité de création d'élément chimique.
...
Le code ci-dessous présente le backend du contrôleur d'éléments chimiques, plus précisément la fonctionnalité de listing des éléments chimiques.
...
\begin{figure}[H]
	...
	\caption{Code backend du contrôleur de listing des éléments chimiques}
\end{figure}
...
La figure ci-dessous présente le résultat du test unitaire du cas d'utilisation de listing des éléments chimiques et révèle que le test est concluent.
...
\begin{figure}[H]
	...
	\caption{Code backend du modèle de création des réactions chimiques}
\end{figure}

La figure ci-dessous présente le code backend du contrôleur des réactions chimiques, plus précisément la fonctionnalité de création des réactions chimiques.
...
\begin{figure}[H]
	...
	\caption{Code backend du contrôleur de création des réactions chimiques}
\end{figure}
...
La figure ci-dessous présente le résultat du test unitaire du cas d'utilisation de création de réactions chimiques et révèle que le test est concluent.
...
Le code ci-dessous présente le backend du contrôleur d'éléments chimiques, plus précisément la fonctionnalité de listing des réactions chimiques.
...
\begin{figure}[H]
	...
	\caption{Code backend du contrôleur de listing des réactions chimiques}
\end{figure}

Le code ci-dessous présente le test unitaire de la fonctionnalité de listing des réactions chimiques.

Ce test unitaire permet de s’assurer que toutes les réponses aux requêtes de listing des réactions chimiques sur le serveur soient des réponses de type succès et que le contenu de la réponse soit une liste de réaction chimique.
...
La figure ci-dessous présente le résultat du test unitaire du cas d'utilisation de listing des réactions chimiques et révèle que le test est concluent.

\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{img/utrcr}
	\caption{Résultat du test unitaire de la fonctionnalité de listing des réactions chimiques}
\end{figure}
